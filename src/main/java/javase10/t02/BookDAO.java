...
public class BookDAO extends AbstractDAO {
    ...
    private void deleteBooksByLine(String line, String request) {
        try {
            PreparedStatement ps = connection.prepareStatement(request);
            ps.setString(1, line);
            ps.executeUpdate();
    ...
    public void deleteBooksByTitle(String title){
        deleteBooksByLine(title, SQL_DELETE_BOOKS_BY_TITLE);
    }

    public void deleteBooksByAuthor(String author){
        deleteBooksByLine(author, SQL_DELETE_BOOKS_BY_AUTHOR);
    }
